...
int ONScripter::scrollableConfigCommand() {
	...
	if (script_h.compareString("divider"))
		...
		imgscalex = true;
	else if (script_h.compareString("imgscaley")) // W_TEMP
		imgscaley = true;
		...
		elembg = true;
	else if (script_h.compareString("elemimg")) // W_TEMP
		elemimg = true;
	...
	if (divider || elembg || scrollbar || elemimg) { // Specify another spriteID to be used within the scrollable
		...
		if (elemimg) // W_CUSTOM
			si.elementImg = sprite2_info[sprite].gpu_image ? &sprite2_info[sprite] : &sprite_info[sprite];
		if (scrollbar) {
		...
		si.imgScaleX = script_h.readInt();
	else if (imgscaley) // W_TEMP
		si.imgScaleY = script_h.readInt();
